...
def enable_CRC16(address, bus):
    ...
    old_settings = bus.read_byte_data(address, 0xCB)  # read as binary number
    ...
    # check
    sleep(1)
    checkdata = bus.read_byte_data(address, 0xCB)  # read as binary number
...
def set_active_buffer_idx(address, bus, index):
    ...
    old_settings = bus.read_byte_data(address, 0xC6)  # read as binary number
    print(bin(old_settings))
    print(old_settings)

    bus.write_byte_data(address, 0xC5, 0b00000000)  

    sleep(1)
    print("...new index settings:")
    new_settings = bus.read_byte_data(address, 0xC6)  # read as binary number
    print(bin(new_settings))
    print(new_settings)
